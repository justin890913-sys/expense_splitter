...
export default function ExpenseSplitter() {
  ...
  const handleLogin = async () => {
    try {
      ...
      alert("登入失敗，請檢查 Firebase 設定或網路連線");
    }
  };
  ...
  // --- 4. 導覽列組件 (包含紅色測試邊框確保能看見) ---
  const Navbar = () => (
    <nav 
      style={{ borderBottom: '4px solid #ef4444' }} // 強制紅框
      className="flex justify-between items-center mb-6 bg-white p-4 rounded-2xl shadow-sm border border-indigo-50"
    >
        ...
        {view === "list" ? (
          <div className="space-y-4">
            <div className="flex justify-between items-center px-1">
              ...
              <button 
                ...
                className="bg-indigo-600 text-white px-4 py-2 rounded-xl font-bold flex items-center gap-2 shadow-lg"
              >
            ...
            {!user ? (
              ...
              <div className="grid gap-3">
                {records.map(record => (
                  ...
                  >
                    <div>
                      <h3 className="font-bold text-gray-800 group-hover:text-indigo-600">{record.name || "未命名紀錄"}</h3>
          ...
          <div className="bg-white rounded-3xl shadow-xl p-5 border border-indigo-50">
            <div className="flex items-center justify-between mb-6">
              ...
              {user && (
                <button onClick={() => saveToCloud()} disabled={autoSaving} className="text-sm font-bold text-indigo-600 flex items-center gap-1">
                  {autoSaving ? "儲存中..." : <><Save className="w-4 h-4" /> 儲存到雲端</>}
                </button>
              )}
            </div>
            ...
            <section className="mb-8">
              ...
              {members.map((m, i) => (
                <div key={i} className="flex gap-2 mb-2">
                  ...
                  <button onClick={() => setMembers(members.filter((_, idx) => idx !== i))} className="text-gray-300 hover:text-red-500">
              ...
              <button 
                ...
                className="w-full py-2 border-2 border-dashed border-gray-100 rounded-xl text-gray-400 text-xs font-bold hover:bg-gray-50"
              >
                + 新增成員
              </button>
            </section>
            ...
            <section>
              ...
              <p className="text-center py-4 text-gray-400 text-sm italic">請在此加入費用計算邏輯...</p>
              <button 
                onClick={() => alert("功能開發中")}
